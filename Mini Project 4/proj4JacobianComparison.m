...
figure();plot((1:N),t,'^',(1:N),t1,'o',(1:N),t2,'x','linewidth',2);
xlabel('run #');ylabel('sec');
title('Jacobian computation time');
legend('Iterative method','MATLAB','symbolic');
axis([1 N 0 .002]);

figure();plot((1:N),diffJ1,'^',(1:N),diffJ2,'o');
xlabel('run #');ylabel('Difference');
title('Jacobian Computation Difference');
legend('Error with MATLAB','Error with Symbolic Expression');
%axis([1 N 0 .002]);
